...
namespace UltimateTemperatureLibrary.UnitTests.Utils
{
	public class TemperatureUnitParserTests
	{
		...
		public class ParseTests
		{
			...
			[TestMethod]
			public void InvalidScaleStringTest() => Assert.AreEqual(new Kelvin(), TemperatureUnitParser.Parse((string) null));

			[TestCategory(TestCategory.BasicTests)]
			[TestMethod]
			[ExpectedException(typeof(FormatException))]
			public void InvalidFormatStringTest() => TemperatureUnitParser.Parse("33,33,33");
			...
			public void UnitIdentificationTest(UInt32 testNumber, string stringToParse) => Assert.AreEqual(Constants.AbsoluteZeroInKelvin, TemperatureUnitParser.Parse(stringToParse).ToKelvin().Value);
		}
	}
}
